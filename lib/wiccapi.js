...
var DexSettleTx = require('./transaction/dexsettletx')
...
WiccApi.prototype.getAddressFromMnemonicCode = function(mnemonic) {
  ...
  }else if(txType == WiccApi.DEX_SETTLE_TX) {
    var dexsettleTx = new DexSettleTx(txData)
    return dexsettleTx.SerializeTx(privkey)
}
  }
  ...
  WiccApi.UCOIN_CONTRACT_INVOKE_TX=12;
  WiccApi.UCOIN_TRANSFER_TX = 13;
  WiccApi.CDP_STAKE_TX = 21;
  WiccApi.CDP_REDEEMP_TX = 22;
  WiccApi.CDP_LIQUIDATE_TX = 23;
  WiccApi.PRICE_FEED_TX = 31;
  WiccApi.FCOIN_STAKE_TX = 53;

  WiccApi.DEX_SETTLE_TX            = 61;  //!< dex settle Tx
  WiccApi.DEX_CANCEL_ORDER_TX      = 62;  //!< dex cancel order Tx
  WiccApi.DEX_BUY_LIMIT_ORDER_TX   = 63;  //!< dex buy limit price order Tx
  WiccApi.DEX_SELL_LIMIT_ORDER_TX  = 64;  //!< dex sell limit price order Tx
  WiccApi.DEX_BUY_MARKET_ORDER_TX  = 65;  //!< dex buy market price order Tx
  WiccApi.DEX_SELL_MARKET_ORDER_TX = 66; //!< dex sell market price order Tx
