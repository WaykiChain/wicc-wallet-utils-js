...
WriterHelper.prototype.writeCdpAsset = function (map) {
  ...
  WriterHelper.prototype.writeAssetData = function (assetData, network) {
    this.writeString(assetData.tokenSymbol)
    var addr = Address.fromString(assetData.ownerAddress, network, 'pubkeyhash')
    var size = addr.hashBuffer.length
    this.writeUInt8(size)
    this.write(addr.hashBuffer)
    this.writeString(assetData.tokeName)
    var minTable = 1
  ...
  WriterHelper.prototype.writeassetUpdateData = function (assetUpdateData, network) {
    ...
    switch (type) {
      case 1:
        this.writeUInt8(1)
        var addr = Address.fromString(assetUpdateData.updateValue, network, 'pubkeyhash')
        var size = addr.hashBuffer.length
        this.writeUInt8(size)
        this.write(addr.hashBuffer)
        break;
